...
export const followFarcasterAction: Action = {
  ...
  handler: async (
    ...
    elizaLogger.info("[NeynarDebug] FOLLOW_FARCASTER: cycle started");
    ...
    elizaLogger.info(
      `[NeynarDebug] FOLLOW_FARCASTER: config — maxFollows=${maxFollowsPerCycle} spamMinFollowers=${spamMinFollowers}`
    );
    ...
    elizaLogger.info(
      `[NeynarDebug] FOLLOW_FARCASTER: state loaded — ${followState.followedFids.length} currently followed, ` +
    ...
    elizaLogger.info(
      `[NeynarDebug] FOLLOW_FARCASTER: ${watchlistFids.length} watchlist candidates`
    );
    ...
    for (const fid of candidates) {
      if (toFollow.length >= maxFollowsPerCycle) {
        elizaLogger.info(
          `[NeynarDebug] FOLLOW_FARCASTER: hit max follows (${maxFollowsPerCycle}), stopping candidate processing`
        );
        break;
      }
      ...
      if (!user) {
        elizaLogger.debug(
          `[NeynarDebug] FOLLOW_FARCASTER: lookup failed for FID ${fid} — skipping`
        );
        continue;
      }
      ...
      if (user.follower_count < spamMinFollowers) {
        stats.lowFollowers++;
        elizaLogger.debug(
          `[NeynarDebug] FOLLOW_FARCASTER: FID ${fid} @${user.username} ` +
      ...
      if (isSpamPattern(user.username, user.profile?.bio?.text)) {
        stats.spamFiltered++;
        elizaLogger.debug(
          `[NeynarDebug] FOLLOW_FARCASTER: FID ${fid} @${user.username} matched spam pattern — skipping`
        );
        continue;
      }
      ...
      toFollow.push(fid);
      elizaLogger.debug(
        `[NeynarDebug] FOLLOW_FARCASTER: candidate accepted FID ${fid} @${user.username}`
      );
    }

    elizaLogger.info(
      `[NeynarDebug] FOLLOW_FARCASTER: filtering complete — ` +
    ...
    if (toFollow.length > 0) {
      elizaLogger.info(
        `[NeynarDebug] FOLLOW_FARCASTER: following ${toFollow.length} users: [${toFollow.join(", ")}]`
      );
      ...
      if (followResult.success && followResult.followed.length > 0) {
        ...
        elizaLogger.info(
          `[NeynarDebug] FOLLOW_FARCASTER: successfully followed ${followResult.followed.length} users`
        );
      }
      ...
      if (followResult.errors.length > 0) {
        ...
        elizaLogger.warn(
          `[NeynarDebug] FOLLOW_FARCASTER: ${followResult.errors.length} follow errors: ${result.errors.join("; ")}`
        );
      }
    } else {
      elizaLogger.info("[NeynarDebug] FOLLOW_FARCASTER: no candidates to follow this cycle");
    }
    ...
    elizaLogger.info(
      `[NeynarDebug] FOLLOW_FARCASTER: cycle #${followState.followCycleCount} complete — ` +
